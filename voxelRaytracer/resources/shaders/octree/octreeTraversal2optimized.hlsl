...
#define GET_PARENT_OCTANT(value) ((value.y & 0x08000000) >> 24)
    ...
    while (stackPointer > 0)
    {
        ...
        if (currentNode.children & (1 << initialOctantOffset))
        {
            if (currentScale == 2)
            {
                ...
                result.item.color = float3(1, 1, 1);
            ...
            OctreeNode myNode = getNodeAtIndexAndOffset(currentNode.childrenIndex, initialOctantOffset);
        ...
        while (!isPositionNotInOctantAtScale(myRay.origin, octantCorner, currentScale) && !done)
        {
            ...
            if (currentNode.children & (1 << currentOctantOffset))
            {
                ...
                OctreeNode myNode = getNodeAtIndexAndOffset(currentNode.childrenIndex, currentOctantOffset);
                TraversalItem myItem;
                myItem.aNode = myNode;
                myItem.aScale = currentScale >> 1;
                traversalStack[stackPointer] = myItem;
